/**
 * @file HealthDiag.h
 * @brief Verbose health diagnostic helpers for ADS1115 examples.
 *
 ...
 */
inline const char* stateToString(ADS1115::DriverState state) {
  switch (state) {
    case ADS1115::DriverState::UNINIT:   return "UNINIT";
    case ADS1115::DriverState::READY:    return "READY";
    case ADS1115::DriverState::DEGRADED: return "DEGRADED";
    case ADS1115::DriverState::OFFLINE:  return "OFFLINE";
 ...
 */
inline const char* errToString(ADS1115::Err err) {
  switch (err) {
    case ADS1115::Err::OK:                   return "OK";
    case ADS1115::Err::NOT_INITIALIZED:      return "NOT_INITIALIZED";
    case ADS1115::Err::INVALID_CONFIG:       return "INVALID_CONFIG";
    case ADS1115::Err::I2C_ERROR:            return "I2C_ERROR";
    case ADS1115::Err::TIMEOUT:              return "TIMEOUT";
    case ADS1115::Err::INVALID_PARAM:        return "INVALID_PARAM";
    case ADS1115::Err::DEVICE_NOT_FOUND:     return "DEVICE_NOT_FOUND";
 ...
 */
inline const char* stateColor(ADS1115::DriverState state) {
  switch (state) {
    case ADS1115::DriverState::READY:    return LOG_COLOR_GREEN;
    case ADS1115::DriverState::DEGRADED: return LOG_COLOR_YELLOW;
    case ADS1115::DriverState::OFFLINE:  return LOG_COLOR_RED;
    case ADS1115::DriverState::UNINIT:   return LOG_COLOR_GRAY;
 ...
 */
inline void printHealthOneLine(ADS1115::ADS1115& driver) {
  ADS1115::DriverState st = driver.state();
 ...
 */
inline void printHealthVerbose(ADS1115::ADS1115& driver) {
  ADS1115::DriverState st = driver.state();
  ADS1115::Status lastErr = driver.lastError();
  uint32_t now = millis();
...
struct HealthSnapshot {
  ADS1115::DriverState state;
  uint8_t consecutiveFailures;
  uint32_t totalSuccess;
  uint32_t totalFailures;
  uint32_t timestamp;

  void capture(ADS1115::ADS1115& driver) {
    state = driver.state();
...
public:
  ...
  void begin(uint32_t intervalMs = 1000) {
    ...
    _lastLogMs = 0;
    _lastState = ADS1115::DriverState::UNINIT;
    _lastConsecFail = 0;
  }
   ...
   */
  void tick(ADS1115::ADS1115& driver, bool forceLog = false) {
    uint32_t now = millis();
    ADS1115::DriverState currentState = driver.state();
...
private:
  ...
  ADS1115::DriverState _lastState = ADS1115::DriverState::UNINIT;
